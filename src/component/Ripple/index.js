    ...
    <div className="w-80p lg:w-65p xl:w-60p">
      <div className="w-full bg-paymentModalBg p-4">
        <div className="flex flex-row items-center ">
          <img src={images.ripple} className="w-10 h-10 bg-transparent" />
        ...
        <div className="w-full mt-5 flex flex-col md:flex-row items-center ">
          <div className="h-[200px] w-[250px] p-4 bg-white flex flex-row items-center justify-center">
            {loader ? (
              <BarLoader color={"#0CAAF3"} />
            ) : (
              <img src={images.ripple} className="h-full w-full" />
            )}
          </div>
          <div className="w-full ml-4 ">
            <div className="text-sm text-white">Ripple Address</div>
            ...
            <div className="text-sm text-white mt-2">Tag</div>
            <div className="text-white text-xs rounded-md p-2 mt-1 bg-inputBG flex flex-row items-center justify-between">
              <span>{tag}</span>
              <img
                src={copiedTag ? images.done : images.copy}
                className="w-3 cursor-pointer mr-1"
                onClick={() => copyToClipboardTag(tag)}
              />
            </div>
            ...
            <div className="opacity-90 border-l-2 border-yellow p-2 text-sm text-yellow bg-inputBG mt-2 font-semibold">
              Only deposit over the Ripple Network!
            </div>
          </div>
        </div>
      </div>
